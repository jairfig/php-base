...
if [ "${DB_CONNECTION:-mysql}" = "oracle" ]; then
  echo "⏳ Aguardando Oracle em ${DB_HOST:-oracle}:${DB_PORT:-1521}..."
  TRIES=0
  until php -r '
    $h=getenv("DB_HOST")?: "oracle";
    $p=getenv("DB_PORT")?: "1521";
    $s=getenv("DB_SERVICE_NAME")?: "ORCLPDB1.localdomain";
    $u=getenv("DB_USERNAME")?: "system";
    $pw=getenv("DB_PASSWORD")?: "oracle";
    $c=@oci_connect($u,$pw,"$h:$p/$s","AL32UTF8");
    if ($c) { echo "OK"; } else { http_response_code(1); }
  ' >/dev/null 2>&1
  do
    TRIES=$((TRIES+1))
    if [ "$TRIES" -gt 60 ]; then
      echo "❌ Timeout aguardando Oracle."
      break
    fi
    sleep 2
  done
  ...
  echo "⏳ Aguardando MySQL em ${DB_HOST:-db}:${DB_PORT:-3306}..."
  # (seu bloco existente de espera do MySQL aqui)
fi
